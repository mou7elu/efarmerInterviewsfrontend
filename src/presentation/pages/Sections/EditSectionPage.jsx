/**
 * Edit Section Page
 * Modifier une section existante (titre, ordre, voletId)
 */
...
const EditSectionPage = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Charger les volets et la section en parallèle
        const [voletsData, sectionData] = await Promise.all([
          voletsAPI.getAll(),
          sectionsAPI.getById(id)
        ]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await sectionsAPI.update(id, dataToSubmit);
      navigate('/sections', { state: { message: 'Section modifiée avec succès' } });
    } catch (err) {
  ...
  return (
    <Container maxWidth="sm">
      <Box mb={4}>
        <Typography variant="h4">Modifier la section</Typography>
