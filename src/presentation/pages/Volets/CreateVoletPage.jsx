/**
 * Create Volet Page
 * Formulaire de création d'un volet (titre, ordre, questionnaireId)
 */
...
const CreateVoletPage = () => {
  ...
  const loadQuestionnaires = async () => {
    try {
      ...
      setQuestionnaires(response.data || response);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await voletsAPI.create(formData);
      navigate('/volets', { state: { message: 'Volet créé avec succès' } });
    } catch (err) {
  ...
  return (
    <Container maxWidth="sm">
      <Box mb={4}>
        <Typography variant="h4">Créer un nouveau volet</Typography>
        <Typography variant="body2" color="text.secondary">Titre, ordre et questionnaire</Typography>
      </Box>
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} sm={8}>
              <TextField
                ...
                disabled={isLoading || loadingQuestionnaires}
              >
            ...
            <Grid item xs={12} display="flex" justifyContent="flex-end" gap={2}>
              ...
              <Button type="submit" variant="contained" startIcon={isLoading ? <CircularProgress size={18} /> : <SaveIcon />}>
                {isLoading ? 'Enregistrement...' : 'Créer le volet'}
              </Button>
            </Grid>
          </Grid>
        </Box>
...
export default CreateVoletPage;
