/**
 * Create Profile Page
 * Créer un nouveau profil avec permissions
 */
...
const CreateProfilePage = () => {
  ...
  useEffect(() => {
    // Charger la liste des menus (mock)
    const loadMenus = async () => {
      try {
        ...
        // Initialiser les permissions pour chaque menu
        const initialPermissions = mockMenus.map(menu => ({
          menuId: menu._id,
          canAdd: false,
          canEdit: false,
          canDelete: false,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setIsLoading(true);
      // TODO: appeler l'API pour créer le profil
      await new Promise((r) => setTimeout(r, 800));
      navigate('/profiles', { state: { message: 'Profil créé avec succès' } });
    } catch (err) {
    ...
    <Container>
      <Box mb={4}>
        ...
        <Typography variant="body2" color="text.secondary">
          Définir un profil utilisateur avec ses permissions d'accès
        </Typography>
      </Box>
